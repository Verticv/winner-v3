    ...
    function Cells({ items, count }) {
        return items.map(item => (
            <div 
                key={item.id} 
                style={{
                    backgroundColor: item.id % 2 === 0 ? "#fff" : "#f7f7f7", borderColor: "#dddddd"
                }} 
                className="flex items-center text-14px tracking-tight text-r666666 h-56px w-full border-b font-roboto last:border-b-0 pt-px"
            >
                ...
                <div style={{marginLeft:"0", width:"255px",color:'#0072bc'}} className=" flex justify-center">{item.result}</div>
      ...
      <div
        ...
        <div
          ...
          className="h-54px flex items-center">    
              ...
              <span className="text-white  text-20px ml-4px pt-2px truncate" style={{maxWidth: "950px"}}>파워볼</span>
          </div>
