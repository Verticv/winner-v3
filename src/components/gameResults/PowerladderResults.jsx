...
const PowerladderResults = ({ title = "파워사다리", icon }) => {
  ...
  const CardContent = ({ bet, team1, team2, stat1 = "1.95", stat3 = "1.95" }) => (
    <div className="flex items-center w-full h-54px">
      <div
        style={{ width: "75px", letterSpacing: "-0.015em" }}
        className="flex justify-center font-roboto text-14 text-gray-c8c8c8 mt-3px"
      >
        211
      </div>
      <div className="flex space-x-4px">
        ...
        >
          <div
            ...
            className={`flex items-center justify-between h-34px rounded-3px px-8px pt-px`}
          >
            <span className="truncate w-323px text-left font-spoqaMedium text-14px">{team1}</span>
        ...
        >
          ...
          >
            <div className="flex items-center space-x-5px">
              <span className="text-14px">{stat3}</span>
            </div>
            <span className="truncate w-323px text-14px text-right">{team2}</span>
          </div>
        </button>
      </div>
    </div>
  ...
  function Cells({ items }) {
    return items.map((item) => (
      <div
        key={item.id}
        style={{
          background: item.id % 2 === 0 ? "#fff" : "#f7f7f7",
          borderColor: "#dddddd",
          letterSpacing: "-0.015em",
        }}
        className="flex items-center text-r444444 h-55px w-full border-b  text-14px font-roboto"
      >
    ...
    >
      ...
      >
        ...
        <div className="flex items-center flex-shrink-0 -mt-3px">
          ...
          <span className="ml-5px text-16px text-white pt-px" style={{ letterSpacing: "-0.015em" }}>
            2021-06-29 15:45
          </span>
        </div>
      </div>
    ...
    >
      <div className="w-full rounded-10px bg-white overflow-hidden">
        ...
        >
          ...
          <div style={{ width: "424px", marginLeft: "0" }} className="flex justify-center">
            승(홈)
          </div>
          <div style={{ width: "82px", marginLeft: "4px" }} className="flex justify-center">
            VS
          </div>
          <div style={{ width: "424px", marginLeft: "4px" }} className="flex justify-center">
            패(원정)
          </div>
        </div>
