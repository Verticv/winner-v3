...
const InboxTable = ({ array, checkedState, setCheckedState }) => {
  ...
  function InboxList({ items }) {
    return items.map((item) => (
      <div
        key={item.id}
        className={` text-14px tracking-tight text-r666666 h-56px w-full border-b hover:font-bold last:border-b-0`}
          style={{background: item.isRead === false?'#ecd7fa': item.id % 2 === 0? '#fff':'#f7f7f7',borderColor:"#dddddd",color:item.isRead?'#444444':'#666666'}}
      >
        <div className="flex items-center text-14px tracking-tight h-56px border-b px-52px last:border-b-0">
          ...
          <button className="rounded-4px  text-14px tracking-tight pt-2px flex-shrink-0 hover:filter hover:brightness-125 ml-11px"
          style={{height:"30px", width: "49px",background: '#8458cd',boxShadow: '0px 2px 2px 0px rgba(0, 0, 0, 0.3)',color:'#ededeb'}}>
  ...
  return (
    ...
    >
      <div className="rounded-10px overflow-hidden">
        <div className="
        font-medium text-14px tracking-tight h-55px flex items-center border-b
        px-52px" style={{ background: "linear-gradient(to right, #9d3bbb, #5423a0)" , color:"#eeeeee",borderColor:"#dddddd"}}>
          <div style={{width:'28px'}} className="flex justify-center">선택</div>
