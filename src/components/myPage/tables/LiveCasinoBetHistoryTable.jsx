...
const LiveCasinoBetHistoryTable = ({array, checkedState, setCheckedState, isPopupOpen, setPopupOpen}) => {
    ...
    function Cells({ items }) {
        ...
        return items.map((item,index) => (
          <div className={` text-r666666 font-spoqa text-14px tracking-tight h-56px w-full  flex items-center border-b border-dark-252525 last:border-b-0`}
            style={{background: index%2===0? '#fff':'#f7f7f7',borderColor:"#dddddd"}}>
               ...
               <div style={{width: "185px"}} className="font-robotoRegular text-center">{item.time}</div>
                ...
                <div style={{width: "135px",  WebkitTextStroke: item.profit.includes("+") && "0.2px", color: item.profit.includes("+") ? "#e9441d" : "#666666"}} className="font-robotoRegular text-right">{item.profit}</div>
                <div style={{width: "51px",  WebkitTextStroke: item.status === "승" &&"0.2px" ,color: item.status === "승" ? "#f04281" : "#666666"}} className="text-center">{item.status}</div>
      ...
      <div
       style={{ background: "linear-gradient(to top, #ededeb, #cb78e6)" }}
        ...
        <div className="rounded-10px overflow-hidden">
              <div className="font-spoqaMedium text-14px tracking-tight text-gray-ccc2b6 h-55px flex items-center pt-px border-b "
